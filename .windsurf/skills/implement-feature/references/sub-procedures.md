...
> 在阶段 4.2 执行 E2E 测试时，**调用 `e2e-test` Skill**。
>
> 该 Skill 使用本地 Playwright CLI（`npx playwright test`）而非 Playwright MCP 手动交互。
> 详见 `.windsurf/skills/e2e-test/SKILL.md` 和 `references/playwright-setup.md`。
>
> 核心流程：环境准备 → 编写 spec → 启动服务 → `npx playwright test` → 报告结果
