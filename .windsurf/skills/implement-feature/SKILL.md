...
> 禁止在未启动前后端服务、未执行 Playwright 测试的情况下进入 4.3 验收标准检查。
> 以下 4.2.1 ~ 4.2.5 每一步都是必须的，不得跳过或简化。

#### 4.2.1 端口与 CORS 一致性检查（首先执行）

在启动任何服务前，**必须先读取配置文件确认端口一致性**：

1. 读取 `csr_magic_frontend/vite.config.ts` 中的 `server.port`（默认 3000）
2. 读取 `csr_magic_backend/.../SecurityConfig.java` 中 `corsConfigurationSource()` 的 `setAllowedOrigins`
3. **验证**：前端端口必须在 CORS 白名单中，否则所有 API 请求会被浏览器拦截
4. 如果不一致，修正启动命令使用配置文件中声明的端口

> **教训**：如果前端配置端口是 3000 但用 `--port 5173` 启动，而后端 CORS 只允许 `localhost:3000`，
> 将导致所有浏览器内 API 调用失败（控制台报 CORS 错误），但 Playwright `page.evaluate` 中的
> `fetch` 由于同源策略仍然会被拦截。
...
```
Step 1: 检查端口占用
  netstat -ano | findstr ":8080 :{frontend_port}"

Step 2: 如果后端未运行 → 启动后端
  cwd: csr_magic_backend/
  mvn spring-boot:run --no-transfer-progress
  （非阻塞，WaitMsBeforeAsync=15000，等待 Spring Boot 启动）
  确认日志出现 "Tomcat started on port 8080"

Step 3: 如果前端未运行 → 启动前端（使用配置文件中的端口）
  cwd: csr_magic_frontend/
  npx vite
  （不要手动指定 --port，让 vite.config.ts 决定端口）
  （非阻塞，WaitMsBeforeAsync=8000）
  确认日志出现 "ready in" + 正确端口号

Step 4: 二次确认
  netstat -ano | findstr "LISTENING" | findstr ":8080 :{frontend_port}"
  两个端口都必须出现 LISTENING 状态
```

#### 4.2.3 E2E 认证准备（关键子流程）

大部分页面需要登录态。**必须在测试前完成认证准备**，按以下步骤：

1. **导航到登录页**：`mcp7_browser_navigate` 到 `http://localhost:{frontend_port}/login`
2. **检查是否有可用测试账号**：
   - 使用 `mcp1_query` 查询：`SELECT username, role FROM csr_v2.users LIMIT 10;`
   - 如果没有可用账号，先通过登录页的"注册"链接注册一个测试账号
3. **通过 UI 登录**（不要用 `page.evaluate` 设置 localStorage）：
   - 填写用户名密码 → 点击登录按钮 → 等待跳转
   - **原因**：直接设置 localStorage 不会触发 Zustand store 的 `setAuth()`，
     导致 `isAuthenticated` 仍为 false，PrivateRoute 会重定向到 /login
4. **角色检查与提升**（统一使用 MCP DB-postgres）：
   - 使用 `mcp1_query` 查询用户角色：`SELECT id, username, role FROM csr_v2.users WHERE username = '...';`
   - 如果功能需要 ADMIN 角色（如事件管理、活动管理），使用 `mcp1_query` 提升：
     `UPDATE csr_v2.users SET role = 'ADMIN' WHERE username = '...' RETURNING id, username, role;`
   - **角色提升后必须重新登录**，因为 JWT payload 中的 role 不会自动更新
5. **登录后使用 SPA 内导航**：
   - ❌ 错误：`mcp7_browser_navigate` 到目标页（全页面刷新，Zustand 状态丢失）
   - ✅ 正确：通过侧边栏链接 `mcp7_browser_click` 导航（SPA 路由，状态保持）
   - 如果必须用 `navigate`，需要确认应用有 `loadFromStorage()` 自动恢复逻辑

> **数据库操作规则**：所有 E2E 测试中的数据库查询和修改，统一使用 `mcp1_query`（MCP DB-postgres），
> 不使用 `psql` 命令行。注意表名需加 `csr_v2.` schema 前缀。

#### 4.2.4 Playwright 交互最佳实践

| 问题 | 解决方案 |
|------|---------|
| 元素在视口外无法点击 | 用 `mcp7_browser_evaluate` 执行 `btn.click()`（JS 原生点击不受视口限制） |
| `page.goto()` 后登录态丢失 | 使用 SPA 内导航（点击侧边栏链接），或重新通过 UI 登录 |
| 表单 `fill` 后未触发 React state 更新 | 用 `mcp7_browser_fill_form` 而非 `mcp7_browser_type` |
| 下拉选择框无法用 click 选中 | 用 `mcp7_browser_select_option` 或 `mcp7_browser_fill_form` 的 combobox 类型 |
| 需要检查不可见的 DOM 内容 | 用 `mcp7_browser_evaluate` 执行 `document.querySelector().textContent` |
| 网络请求结果不确定 | 用 `mcp7_browser_wait_for` 等待特定文本出现，或 `time` 等待固定秒数 |
| PowerShell 下 mvn `-D` 参数解析失败 | 用引号包裹：`"-Dtest=com.csr.activity.**"` |

#### 4.2.5 测试执行与结果

1. **测试场景设计**：根据 spec-*.md 验收标准，设计对应的 E2E 测试场景，至少覆盖：
   - 页面渲染正确性（UI 元素存在）
   - 正常业务流程（提交表单、跳转、数据展示）
   - 异常流程（表单验证、错误提示、权限拦截）
   - **边界场景**（Token 过期/无效/缺失时的行为）
2. **执行测试**：使用 Playwright MCP 工具按场景逐一执行
3. **输出结果**：以表格形式汇总每个测试场景的通过/失败状态
...
- **E2E 测试用 Playwright MCP**：本项目 E2E 测试通过 Playwright MCP 工具实时执行，不依赖 `@playwright/test` 包
