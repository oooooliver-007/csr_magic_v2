...
| 2026-05-21 | notification/admin-notification-management | Admin 标记通知已读直接修改员工 is_read 字段（全局管理态），而非维护独立的 admin_read_status 表 | 业务语义是"管理员代表平台确认该通知已被关注"，直接更新员工状态最简洁，无需新增关联表或通知副本 |
| 2026-05-21 | notification/admin-notification-management | NotificationResponse DTO 扩展 userId/username/displayName 字段，沿用同一 record（不新增 AdminNotificationResponse） | admin 端需要展示接收用户信息；复用同一 DTO 减少类型膨胀，员工端调用时三个字段有值但前端不展示 |
| 2026-05-21 | notification/admin-notification-management | NotificationBell 新增 scope prop（'user' \| 'admin'），admin scope 调用 /admin/unread-count 和 /admin 接口 | 复用同一铃铛组件但数据来源不同；scope 显式声明意图，避免通过 URL 判断上下文 |
| 2026-05-31 | security/p0-hardening-1 | 移除 application.yml 中 JWT_SECRET 和 DATABASE_PASSWORD 的硬编码默认值，改为启动时 CommandLineRunner 校验（ConfigurationValidator） | 防止因遗漏环境变量导致弱密钥上线。Spring 默认占位符 `${VAR:default}` 在未设置时静默使用默认值，替换为 `${VAR}` 后缺失环境变量将启动失败并给出明确指引 |
| 2026-05-31 | security/p0-hardening-2 | Refresh Token 从 localStorage 迁移到 httpOnly SameSite=Lax Cookie；前端移除 refreshToken 的 localStorage 存取，authApi.refresh() 不再手动传参；Vite 添加 `/api` proxy 解决开发环境跨域 Cookie 问题 | 杜绝 XSS 窃取长期凭证的风险。后端 AuthController login/register 设置 Cookie，logout 清除 Cookie；前端仅保留 accessToken 在 localStorage，refresh 依赖浏览器自动发送 Cookie |
| 2026-05-31 | ai-poster/p0-hardening-3 | 后端新增 @Scheduled 定时轮询（fixedDelay=15s），查询所有 PENDING/GENERATING 状态的 AiPoster 并调用 AI 服务同步状态；AiPosterRepository 新增 findByStatusIn 方法 | AI 服务 _task_store 为进程内 Dict，服务重启后状态丢失导致前端轮询永久卡住。后端定时轮询确保即使 AI 服务重启，也能在 15 秒内将任务标记为 FAILED（而非永久卡在 PENDING） |
| 2026-05-31 | notification/admin-entry-redesign | Admin 端通知入口彻底任务化：将顶部通知铃铛改造成待审核报名待办（AdminReviewTodoBell.tsx），数据由新增的 `/api/v2/participations/review-todos` 驱动（状态为 `PENDING` 或 `RE_SUBMITTED` 的报名数）；彻底移除左侧边栏「通知管理」菜单，并重定向旧路由 `/admin/notifications` 至 `/admin/participations?state=PENDING` | 对齐管理员的真实任务场景（即审批待办）。任务化铃铛极大地缩短了审批响应路径，避免信息过载；同时通过重定向机制平滑过渡，且员工端体验完全保留 |
